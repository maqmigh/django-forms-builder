...
from forms_builder.forms.models import Form, Field, FormEntry, FieldEntry, STATUS_ARCHIVED
...
class FieldAdmin(admin.TabularInline):
    ...
    def has_add_permission(self, request, obj=None):
        if obj:
            return obj.status != STATUS_ARCHIVED
        else:
            return super(FieldAdmin, self).has_add_permission(request, obj)

    def has_delete_permission(self, request, obj=None):
        if obj:
            return obj.status != STATUS_ARCHIVED
        else:
            return super(FieldAdmin, self).has_change_permission(request, obj)
...
class FormAdmin(admin.ModelAdmin):
    ...
    def has_change_permission(self, request, obj=None):
        if obj and obj.status == STATUS_ARCHIVED:
            return False
        return super(FormAdmin, self).has_change_permission(request, obj)

    def save_model(self, request, obj, form, change):
        if 'status' in form.changed_data and obj.status == 3:
            fields_to_hash = obj.fields.filter(anonymize=True).values_list('id', flat=True)
            field_entries = FieldEntry.objects.filter(entry__form_id=obj.id, field_id__in=fields_to_hash, hashed=False)
            for fe in field_entries:
                fe.value = hashlib.md5(fe.value.encode("utf-8")).hexdigest()
                fe.hashed = True
